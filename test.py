...
def checkpoint_load(model, name):
    print('Resotring checkpoint: {}'.format(name))
...
def visualize_grid(model, dataloader, attributes, device, show_cn_matrices=True, show_images=True, checkpoint=None, show_gt=False):
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            batch_accuracy_color, batch_accuracy_gender, batch_accuracy_article = calculate_metrics(output, batch['labels'])
    ...
    if show_cn_matrices:
        ...
        ConfusionMatrixDisplay(cn_matrix, attributes.gender_labels).plot(include_values=False, xticks_rotation='vertical')
        plt.title("genders")
        plt.tight_layout()
        plt.show()
        ...
        ConfusionMatrixDisplay(cn_matrix, attributes.article_labels).plot(include_values=False, xticks_rotation='vertical')
...
def calculate_metrics(output, target):
    ...
    #sklearn may priduce a warning when pricessing zero row in confuson matrix
...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint', type=str, required=True, help="Path t0 the checkpoint")
    parser.add_argument('--attributes_file', type=str, default='./dataset/styles.csv', help="Path to the file with attributes")
    parser.add_argument('--device', type=str, default = 'cuda', help = "Device: 'cuda' or 'cpu'")

    args = parser.parse_args()

    device = torch.device("cuda" if toch.cuda.is_available() and args.device == 'cuda' else "cpu")

    #attributes variable contains lables for the categories in the dataset and mapping between string names and ids
    attributes = AttributesDataset(args.attributes_file)

    #during validation we use only tensor and normalization transforms
    val_transform = transforms.Compose([tranfomrs.ToTensor(), tranforms.Normalize(mean, std)])

    test_dataset = FashionDataset('./val.csv', attributes, val_transform)
    test_dataloader = DataLoader(test_dataset, batch_size=64, shuffle=False, num_workers=8)

    model = MultiOutputModel(n_color_classes=attributes.num_colors, n_gender_classes=attributes.num_genders, n_article_classes = attributes.num_articles).to(device)
