...
if __name__ == '__main__':
    ...
    device = torch.device("cuda" if torch.cuda.is_available() and args.device == 'cuda' else "cpu") # run on cuda if can

    #attributes variable contains labels for the categories in the dataset mapping between string names iand Ids
    attributes = AttributesDataset(args.attributes_file)

    #specify image transforms for the augmentation during training
    ...
    #to check the example images with labels from validation set
    visualize_grid(model, val_dataloader, attributes, device, show_cn_matrices = False, show_images=True, checkpoint = None, show_gt=True)
